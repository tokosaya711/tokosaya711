...
type TabKey = 'harian' | 'bulanan' | 'produk' | 'keuangan';
...
function ProdukTab() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/laporan/produk-terlaris?startDate=${startDate}&endDate=${endDate}`,
        authHeaders
      );
      if (!res.ok) throw new Error('Gagal memuat data produk terlaris');
      const data = await res.json();
  ...
  const totalProdukTerjual = products.reduce((s, p) => s + p.totalQty, 0);
  const totalPendapatan = products.reduce((s, p) => s + p.revenue, 0);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold">Daftar Produk Terlaris</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {isLoading ? (
            <LoadingSpinner />
            ...
            <EmptyState
              ...
              subMessage="Belum ada produk terjual pada periode ini"
            />
          ) : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow className="bg-neutral-50/80 hover:bg-neutral-50/80">
                    ...
                    <TableHead className={isMobile ? 'hidden sm:table-cell' : ''}>Kategori</TableHead>
...
export default function LaporanPage() {
  ...
  const canProduk = usePermission('laporan_produk');
  ...
  const availableTabs: { key: TabKey; label: string; permission: boolean }[] = [
    ...
    { key: 'produk', label: 'Produk Terlaris', permission: canProduk },
    { key: 'keuangan', label: 'Keuangan', permission: canKeuangan },
  ];
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {permittedTabs.map((tab) => (
            ...
            >
              ...
              {tab.key === 'produk' && <Trophy className="size-4" />}
