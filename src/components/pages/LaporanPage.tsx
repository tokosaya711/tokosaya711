      ...
      {!error && !isLoading && financialData && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            ...
            <StatCard
              title="Total Modal"
              value={formatRupiah(financialData.totalModal)}
              icon={<ShoppingCart className="h-4 w-4 sm:h-5 sm:w-5" />}
              accentClass="text-red-600"
              iconBgClass="bg-red-50 text-red-500"
            />
          ...
          {financialData.dailyBreakdown && financialData.dailyBreakdown.length > 0 && (
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base font-semibold">
                  Rincian Harian — {formatDate(startDate)} s/d {formatDate(endDate)}
                </CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                <ScrollArea className="max-h-96">
                          ...
                          <TableRow key={day.date}>
                            ...
                            <TableCell className={isMobile ? 'hidden md:table-cell text-right' : 'text-right'}>
                              <span className="text-sm text-orange-600">
                                {formatRupiah(day.diskon)}
                              </span>
                            </TableCell>
                            <TableCell className={isMobile ? 'hidden md:table-cell text-right' : 'text-right'}>
                              <span className="text-sm text-purple-600">
                                {formatRupiah(day.pajak)}
                              </span>
                            </TableCell>
                            <TableCell className="text-right">
          ...
          {financialData.dailyBreakdown && financialData.dailyBreakdown.length === 0 && (
            <EmptyState
              ...
              subMessage={`Belum ada transaksi pada periode ${formatDate(startDate)} — ${formatDate(endDate)}`}
            />
          )}
        </>
      )}
...
export default function LaporanPage() {
  ...
  const availableTabs: { key: TabKey; label: string; permission: boolean }[] = [
    ...
    { key: 'keuangan', label: 'Ringkasan Keuangan', permission: canKeuangan },
  ];
